...
globals [patch-dataset Open Closed Invaded coverMap Iteration VarFileName VarRasterFileName TempFolder patches_out]
...
patches-own [landCover]
  ...
  ;VarFileName %SSIM_VARIABLE_FILENAME%
  ;VarRasterFileName %SSIM_VARIABLE_RASTER_FILENAME%
  ;TempFolder %SSIM_NETLOGO_TEMP_FOLDER%
...
to setup
  ...
  gis:set-world-envelope gis:envelope-of patch-dataset
  setup-patches
  setup-bugs
  file-open %SSIM_VARIABLE_FILENAME%
  file-print csv:to-row (list "Iteration" "Timestep" "OutputVariableID" "Amount")
  file-close
  file-open %SSIM_VARIABLE_RASTER_FILENAME%
  file-print csv:to-row (list "Iteration" "Timestep" "OutputVariableID" "Filename")
  file-close
  reset-ticks
...
to setup-patches
  gis:apply-raster patch-dataset landCover
  ask patches [
    if landCover = 1 [
      set pcolor green
    ]
...
to update-patches
  ask patches [
    ...
    set patches_out gis:patch-dataset landCover
   ]
end

to move-bugs
  ask bugs [
    ifelse any? neighbors with [landCover = 3]
      [move-to one-of patches with [landCover = 3]]
      [right random 360 forward 2]
  ]
end
